...
var ColorScheme = /** @class */ (function () {
    function ColorScheme(name, colorSets, isDistinct) {
        var _this = this;
        this.name = name;
        this.colorSets = [];
        this.isDistinct = !!isDistinct;
        colorSets.forEach(function (set) { return _this.colorSets[set.length] = set; });
    }
    ColorScheme.fromObject = function (name, colorSets) {
        var colorSetsArray = [];
        Object.keys(colorSets).forEach(function (numColors) { return colorSetsArray[+numColors] = colorSets[numColors]; });
        return new ColorScheme(name, colorSetsArray);
    };
    ColorScheme.prototype.improviseGradientFromShorter = function (shortColors, numColors) {
        var newColors = Util_1.clone(shortColors);
        while (newColors.length < numColors) {
            for (var i = newColors.length - 1; i > 0; i -= 1) {
                var startColor = d3_color_1.rgb(newColors[i - 1]);
                var endColor = d3_color_1.rgb(newColors[i]);
                var newColor = d3_interpolate_1.interpolate(startColor, endColor)(0.5);
    ...
    ColorScheme.prototype.improviseGradientFromLonger = function (knownColors, numColors) {
        var newColors = [];
        var scale = interpolateArray(knownColors);
        for (var i = 0; i < numColors; i++) {
            newColors.push(scale(i / numColors));
        }
        return newColors;
    };
    ColorScheme.prototype.getGradientColors = function (numColors) {
        var colorSets = this.colorSets;
        if (colorSets[numColors])
            return Util_1.clone(colorSets[numColors]);
        else {
            var prevColors = Util_1.clone(colorSets).reverse().find(function (set) { return set && set.length < numColors; });
            if (prevColors)
                return this.improviseGradientFromShorter(prevColors, numColors);
            else
                return this.improviseGradientFromLonger(colorSets.find(function (set) { return !!set; }), numColors);
        }
    };
    ColorScheme.prototype.getDistinctColors = function (numColors) {
        var colorSets = this.colorSets;
        if (colorSets[numColors])
            return Util_1.clone(colorSets[numColors]);
        else if (numColors > colorSets.length - 1)
            return this.improviseGradientFromShorter(Util_1.last(colorSets), numColors);
        else {
    ...
    ColorScheme.prototype.getColors = function (numColors) {
        if (this.isDistinct) {
...
exports.default = (function () {
    ...
    for (var schemeKey in colorbrewer) {
        var name_1 = longSchemeNames[schemeKey];
        if (name_1) {
            var colorSets = colorbrewer[schemeKey];
            colorSchemes[schemeKey] = ColorScheme.fromObject(name_1, colorSets);
        }
    }
    // Perceptual color scales from https://github.com/politiken-journalism/scale-color-perceptual
    colorSchemes['Viridis'] = new ColorScheme("Viridis", [
    ...
    colorSchemes['Inferno'] = new ColorScheme("Inferno", [
    ...
    colorSchemes['owid-distinct'] = new ColorScheme("OWID Distinct", [
        ["#3360a9", "#ca2628", "#34983f", "#f6a324", "#a652ba",
            "#2a939b", "#e6332e", "#9ecc8a", "#ffd53e", "#b077b1",
            "#7ec7ce", "#e6332e", "#6bb537", "#f07f59", "#ce8ebd",
            "#5eb77e", "#818282", "#fceb8c", "#9ecc8a", "#cfcd1e",
            "#58888f", "#932834", "#df3c64", "#db2445", "#f9bc8f",
            "#d26e66", "#c8c8c8", "#ed6c2d"]
    ], true);
    colorSchemes['stackedAreaDefault'] = new ColorScheme("OWID 4 Color Gradient", [
        ["#3360a9"],
        ["#3360a9", "#34983f"],
    ...
    colorSchemes['continents'] = new ColorScheme("Continents", [[
            "#5675c1",
            "#aec7e8",
            "#d14e5b",
            "#ffd336",
