...
function build_static($post_ID, $post_after, $post_before) {
	...
	exec("cd " . dirname(__FILE__) . " && node dist/postUpdatedHook.js " . escapeshellarg($current_user->user_email) . " " . escapeshellarg($current_user->display_name) . " " . $post_after->slug . " /tmp/wp-static.log", $op);
}
